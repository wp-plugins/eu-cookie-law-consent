...
function EUCLC_cookieMessage()
{
	...
	$debug = $setting_value['chkDebug'] == 'on';
...
jQuery(function(){ 
  if (navigator.cookieEnabled === true)
  {
    if (document.cookie.indexOf("visited") == -1)
	{
	  ...
	  jQuery('head').append('<style type="text/css">#cookie {<?php echo $absPosition; ?>left:0;top:0;width:100%;height:100%;background:rgb(<?php echo $backgroundColour; ?>);background:rgba(<?php echo $backgroundColour; ?>,<?php echo $backgroundTransparency; ?>);z-index:9999;}#cookie #wrapper {padding:<?php echo $padding; ?>;}#cookie h2 {color:<?php echo $titleColour; ?>;display:block;text-align:center;font-family:<?php echo $titleFont; ?>;font-size:<?php echo $titleSize; ?>}#cookie p {color:<?php echo $messageColour; ?>;display:block;font-family:<?php echo $messageFont; ?>;font-size:<?php echo $messageSize; ?>}#cookie #close{text-align:center;}#closecookie{color:<?php echo $closeColour; ?>;font-family:<?php echo $closeFont; ?>;font-size:<?php echo $closeSize; ?>;text-decoration:none}@media only screen and (min-width: 480px) {#cookie {height:auto;}#cookie #wrapper{max-width:<?php echo $maxWidth; ?>;margin-left:auto;margin-right:auto;}#cookie h2{width:18%;margin-top:0;margin-right:2%;float:left;text-align:right;}#cookie p {width:68%;margin:0 1%;float:left;}#cookie #close{width:9%;float:right;}}</style>');
...
$setting_value = get_option('EUCLC');
$setting_value = $setting_value['chkReset'];
...
$setting_value = get_option('EUCLC');
$setting_value = $setting_value['chkBlock'];
if($setting_value) { $checked = ' checked="checked" '; }
...
$setting_value = get_option('EUCLC');
$setting_value = $setting_value['chkDebug'];
if($setting_value) { $checked = ' checked="checked" '; }
...
function EUCLC_defaults() 
{
    ...
    if ( (!is_array($tmp)) || ($tmp['chkReset']=='on') )
	{
		$arr = array(
